...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        nearest_pos = self.place
        distance = 0

        while nearest_pos is not beehive:
            if self.min_range <= distance <= self.max_range:
                bee_in_pos = random_or_none(nearest_pos.bees)
                if len(nearest_pos.bees) != 0:
                    return random_or_none(nearest_pos.bees)
            nearest_pos = nearest_pos.entrance
            distance += 1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
        	...
        	pos_behind = self.place.exit
        	while pos_behind != None:
        		ant = pos_behind.ant
        		if isinstance(ant, BodyguardAnt):
        			if ant not in self.inspired_ants:
        				self.inspired_ants.append(ant)
        				ant.damage *= 2
        			ant = ant.contained_ant
        		if ant != None and ant not in self.inspired_ants:
        			self.inspired_ants.append(ant)
        			ant.damage *= 2
        		pos_behind = pos_behind.exit
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    curr_action = bee.action
    def new_action(colony):
    	nonlocal duration
    	if duration > 0:
    		effect(curr_action, bee)(colony)
    	else:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and target.not_scared_yet:
        	target.not_scared_yet = False
