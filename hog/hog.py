...
def is_swap(player_score, opponent_score):
    ...
    while opponent_score >= 10:
    	opponent_score //= 10
    ofirst = opponent_score
    return plast * pfirst == olast * ofirst
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
    	if player == 0:
    		...
    		if feral_hogs and abs(lroll0 - roll0) == 2:
    			score0 += 3
    		lroll0 = roll0
    	elif player == 1:
    		roll1 = strategy1(score1, score0)
    		score1 += take_turn(roll1, score0, dice)
    		if feral_hogs and abs(lroll1 - roll1) == 2:
    			score1 +=3
    		lroll1 = roll1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    test = 9
    index = 10
    maxrolls = make_averaged(roll_dice, num_samples)(10,dice)
    while test > 0:
    	if make_averaged(roll_dice, num_samples)(test,dice) > maxrolls:
    		maxrolls = make_averaged(roll_dice, num_samples)(test,dice)
    		index = test
    	test -= 1
    return index
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(free_bacon(opponent_score) + score, opponent_score):
    	if opponent_score < free_bacon(opponent_score) + score:
    		return num_rolls
    	elif opponent_score > free_bacon(opponent_score) + score:
    		return 0
