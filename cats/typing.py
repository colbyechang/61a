...
def about(topic):
    ...
    def f(n):
        for i in topic:
            for s in split(lower(remove_punctuation(n))):
                if i == s:
                    return True
        return False
    return f
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    low = diff_function(user_word, valid_words[0], limit)
    ind = 0
    for x in range(len(valid_words)):
    	check_next = diff_function(user_word, valid_words[x], limit)
    	if low > check_next:
            low = check_next
            ind = x
    if low <= limit:
...
def report_progress(typed, prompt, id, send):
    ...
    for i in range(len(prompt)):
        if i == len(typed) or prompt[i] != typed[i]:
...
def fastest_words(word_times, margin=1e-5):
    ...
    for i in range(n_words):
    	fastest = min([elapsed_time(word_times[player][i + 1]) - elapsed_time(word_times[player][i]) for player in range(n_players)])
    	for p in range(n_players):
    		ptime = elapsed_time(word_times[p][i + 1]) - elapsed_time(word_times[p][i])
    		if ptime - fastest < margin:
    			fastlist[p].append(word(word_times[p][i + 1]))
    return fastlist
    # END PROBLEM 9
